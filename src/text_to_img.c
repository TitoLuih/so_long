...
/*   Updated: 2025/03/12 20:20:52 by lruiz-to         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	text_to_img(t_game *game)
{
	...
	if (!game->t_coin || !game->t_floor || !game->t_player || !game->t_closed_exit 
		|| !game->t_open_exit || !game->t_wall)
	...
	else
	{
		...
		clean_textures(game);
		if (!game->i_coin || !game->i_floor || !game->i_player || !game->i_open_exit
			|| !game->i_closed_exit || !game->i_wall)
